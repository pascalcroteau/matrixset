...
A `matrixset` is a container of matrices (both `matrix` and types from `Matrix`
package), each having the same number of rows and columns and the same dimnames.
Moreover, each dimname must uniquely identify elements.
 ...
 * You can store sparse matrices and other special matrices of package `Matrix`.
   This, among other advantages, can make the object size argument even more
   important.
