...
the opportunity to map the calibration points with their dilution values.
...
pcas <- mrm_plus2015_ratio %>% 
     ...
     apply_matrix(pca = ~ {
         meta <- current_row_info() %>% as.data.frame()
         rownames(meta) <- meta$.rowname
         pca(t(.m), scale = TRUE, metadata = meta)
     }, .matrix = c("light_area", "heavy_area", "area_log_ratio"))

bps <- imap(pcas, ~ biplot(.x$pca, colby = "CalibrationPoint", 
...
diff <- mrm_plus2015_ratio %>% 
    annotate_row(point = case_when(is.na(CalibrationPoint) ~ str_match(.rowname, "(.*)_\\d$")[,2],
                 ...
                 replicate = case_when(is.na(CalibrationPoint) ~ paste("Replicate", str_match(.rowname, ".*_(\\d)$")[,2], sep = "_"),
                                   TRUE ~ Replicate)) %>% 
    apply_column_dfl(p_cov = predict(lmer(.j ~ 1 + SampleType + (1|point)),
                                     type = "response"),
